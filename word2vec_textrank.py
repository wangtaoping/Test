...
def cosine_similarity(vec1, vec2):
    ...
    return cosine_value
def compter_similarity_by_avg(sents_1, sents_2, model):
    """
    对两个句子求平均词向量
    :param sents_1:
    :param sents_2:
...
def create_graph(word_sent, model):
    ...
    for i, j in product(range(num), repeat=2):
        if i != j:
            board[i][j] = compter_similarity_by_avg(word_sent[i], word_sent[j], model)
    return board

def sorted_sentence(graph, sentences, topK):
    """
    调用pagerrank算法进行计算并排序
    :param graph:
    :param sentences:
    :param topk:
    :return:
...
def do(text, topK):
    ...
    print("list_sents",len(list_sents))
